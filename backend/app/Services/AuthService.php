...
public function registerCandidat(array $data): array
{
    ...
    // ✅ Refresh AVANT de créer le token
    $user = User::where('email', $data['email'])->first();

    app(\App\Services\EmailVerificationService::class)->sendCode($user);

    Log::info('✅ Candidat créé', ['user_id' => $user->getKey()]);
    ...
    public function loginAdmin(string $email, string $password): array
{
    ...
    public function findOrCreateSocialUser(
        ...
        if (!$user) {
            ...
            // ✅ Mettre à jour email_verified_at si null
    ...
    // ✅ Refresh le user pour avoir l'id MongoDB correct
    $user = User::where('email', $user->email)->first();

    if (!$user) {
        throw new \Exception('User introuvable pour OAuth redirect');
    }

    $token = $user->createToken('auth_token')->plainTextToken;

    \Illuminate\Support\Facades\Log::info('Token OAuth', [
        'token_preview' => substr($token, 0, 30),
        'user_id'       => (string) $user->getKey(),
        'tokenable_id'  => DB::connection('mongodb')
            ->table('personal_access_tokens')
            ->orderBy('created_at', 'desc')
            ->first()?->tokenable_id ?? 'VIDE',
    ]);
