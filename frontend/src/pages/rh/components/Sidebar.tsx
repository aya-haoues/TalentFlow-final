...
import {
    ...
    AppstoreOutlined,  EyeOutlined, LockOutlined, SafetyOutlined,
...
import { LinkOutlined as ApiOutlined } from '@ant-design/icons';
const { Sider } = Layout;
...
const isSettingsSection = location.pathname.includes('/rh/settings');
    ...
    const settingsItems: MenuItem[] = [
    {
        ...
        type: 'group',
        children: [
            { key: '/rh/settings',                  icon: <SettingOutlined />,      label: 'Notifications'     },
            { key: '/rh/settings/display',           icon: <EyeOutlined />,          label: 'Affichage'         },
            { key: '/rh/settings/automation',        icon: <ThunderboltOutlined />,  label: 'Automatisation'    },
            { key: '/rh/settings/integrations',      icon: <ApiOutlined />,          label: 'Intégrations'      },
            { key: '/rh/settings/privacy',           icon: <SafetyOutlined />,       label: 'Confidentialité'   },
        ],
    },
];
    ...
    if (isSettingsSection)  return settingsItems;   // ← ajouter
...
<Tooltip title="Paramètres" placement="right">
    <SettingOutlined
        style={location.pathname.includes('/rh/settings') ? activeIconStyle : iconBaseStyle}
    ...
    />
            ...
            <Sider width={240} theme="light" style={{ borderRight: '1px solid #e2e8f0', background: '#fff' }}>
                ...
                <div style={{
                    ...
                    {isCandidateSection ? <><UserOutlined /> Candidats</>
                        : isJobSection   ? <><FileSearchOutlined /> Emplois</>
                         : isSettingsSection ? <><SettingOutlined />      Paramètres</>   // ← ajouter
                ...
                <Menu
    mode="inline"
    selectedKeys={[location.pathname]} // Plus besoin de concaténer le hash !
    onSelect={({ key }) => navigate(key)} // Navigation simple
    items={getMenuItems()}
/>
