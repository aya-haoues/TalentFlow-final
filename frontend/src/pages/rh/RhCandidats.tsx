...
const STATUT_CONFIG: Record<string, { color: string; label: string }> = {
    en_cours: { color: '#6B7280', label: 'Nouveau'   },
...
export default function RhCandidats() {
    ...
    const pathFilter = useMemo(() => {
        if (location.pathname.includes('nouveaux'))   return 'en_cours';
                ...
                <Space onClick={e => e.stopPropagation()}>
                    <Tooltip title="Voir le profil">
                        <Button
                            ...
                            icon={<EyeOutlined />}
                            onClick={() => {
    setSelectedCandidate(record);   // données de la liste (instantané)
    setIsModalOpen(true);           // modale visible tout de suite

    api.get(`/rh/applications/${record.id}`)
        .then(res => setSelectedCandidate(res.data.data))
        .catch(() => message.warning('Impossible de charger les détails complets'));
}}
                ...
                <Space style={{ marginBottom: 20 }}>
                    ...
                    <Select
                        ...
                        options={[
                            ...
                            { value: 'en_cours', label: `Nouveaux (${counts.en_cours})` },
