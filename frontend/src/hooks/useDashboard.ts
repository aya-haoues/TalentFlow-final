...
export interface Application {
  id: number;
  statut: 'en_cours' | 'acceptee' | 'refusee' | 'entretien' | 'retiree';
...
export function useDashboard() {
  ...
  const [stats, setStats]               = useState<Stats>({ total: 0, en_cours: 0, acceptee: 0, entretien: 0 });
...
const fetchApplications = useCallback(async (page = 1) => {
  ...
  try {
    const params: any = { 
      page,
      // On envoie le statut sélectionné au backend
    ...
    message.error('Erreur lors du filtrage');
  } finally {
    setLoading(false);
  }
}, [statusFilter, searchQuery]); // La fonction se déclenche quand ces valeurs changent
