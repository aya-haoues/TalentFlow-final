...
export default function RhLayout({ title, description, actions, children }: RhLayoutProps) {
  ...
  // ── User state réactif (se met à jour quand l'avatar change) ────────────
  const [user, setUser] = useState<User>(() => authService.getCurrentUser() as User);

useEffect(() => {
    // Écoute l'événement 'storage' déclenché après upload d'avatar
    const syncUser = () => {
      const updated = authService.getCurrentUser() as User;
  ...
  // ── Initiales de fallback si pas d'avatar ───────────────────────────────
          ...
          <Header style={{
            ...
            <Space size="large">
              ...
              <Dropdown menu={{ items: userMenuItems }} placement="bottomRight" trigger={['click']}>
                <div data-testid="user-avatar" style={{ cursor: 'pointer' }}>
                <Space style={{ cursor: 'pointer', padding: '4px 8px', borderRadius: 8 }}>
                  ...
                  <Avatar
                    ...
                    style={
                      user?.avatar
                        ? { border: `2px solid ${primaryColor}40`, objectFit: 'cover' }
                        : { background: primaryColor, fontWeight: 600 }
                    }
                  >
